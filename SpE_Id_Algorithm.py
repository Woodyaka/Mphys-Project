...
def define_variables(lev_shape, time_shape, lat_shape, lon_shape, LTshape, time_it_shape, ds_months_shape, newlat_shape, intlat_shape, crit_freq_on):
    ...
    # print("SpEs shape:", SpEs.shape)
    # print("SpEs dtype:", SpEs.dtype)
    # print("SpEs memory usage:", SpEs.nbytes / (1024**2), "MB")
    # SpEs = SpEs.astype(np.float32)
    # SpEsns_freq_bool = SpEsns_freq_bool.astype(np.float32)
    # SpEsns_freq = SpEsns_freq.astype(np.float32)
    # SpEsns_freq_time = SpEsns_freq_time.astype(np.float32)
    # SpEsns_Occ_Freq = SpEsns_Occ_Freq.astype(np.float32)
    # SpEsns_Occ_Fr_avg = SpEsns_Occ_Fr_avg.astype(np.float32)
    # SpEsns_Occ_Fr_dsavg = SpEsns_Occ_Fr_dsavg.astype(np.float32)
    ...
    SpEsns = da.empty((lev_shape, time_shape, lat_shape, lon_shape, time_it_shape, ds_months_shape), 
                  dtype=np.float32) # Gets around the memory issues
    
    # SpEsns_freq_bool = SpEsns_freq_bool.astype(np.float32)
    
    # SpEsns_freq_bool = np.empty((lev_shape, time_shape, lat_shape, lon_shape, time_it_shape, ds_months_shape), dtype=np.float32) #SpE occurence in each grid space as True/False 
    SpEsns_freq_bool = da.empty((lev_shape, time_shape, lat_shape, lon_shape, time_it_shape, ds_months_shape), 
                  dtype=np.float32) # Gets around the memory issues
    # SpEsns_freq = np.empty((lev_shape, time_shape, lat_shape, lon_shape, time_it_shape, ds_months_shape), dtype=np.float64) #SpE occurence in each grid space as 1s/0s
    SpEsns_freq = da.empty((lev_shape, time_shape, lat_shape, lon_shape, time_it_shape, ds_months_shape), 
                  dtype=np.float32) # Gets around the memory issues
    SpEsns_freq_time = np.empty((lev_shape, lat_shape, lon_shape, time_it_shape, ds_months_shape), dtype=np.float64)     #total SpE occurences in 2 week time period
    ...
    alt_sl_sh = da.empty((lev_shape, time_shape, lat_shape, lon_shape, time_it_shape, ds_months_shape), 
                  dtype=np.float32) # Gets around the memory issues
...
def save_results(run_name, Monthfolderstr, lev, lev_sl, timear, lat, intlat, lon, LTar, LTlong, time_ar_2wk, ds_months, Zavg_sl, altavg, altavg_sl, times_str_min, times_str_max, SpEs_Occ_Fr_b_dsavg, SpEs_Occ_Fr_b_avg, SpEs_Occ_Fr_bb_5d_dsavg, SpEsns_Occ_Fr_dsavg, SpEs, alt_sl_sh_avg_b_dsavg, alt_sl_sh_avg_b_dsavgLT, alt_sl_sh_avg_bb_5d_dsavg, Mptdens_avv1_b_5d, Mptdens_std, Mptdens_nsstd, SpEsns_freq_time, SpEs_freq_time, Mptdens_avg, Mptdens_nsdiff, Mptdens_nsavg, SpEs_Occ_Freq_llbav, SpEsns_Occ_Freq_llav, SpEsns_Occ_Freq_lat ):
    ...
    output_file = f'Nc_Files/{str(run_name)}_SpE_Output_{str(Monthfolderstr)}_90-150km_{filename_append}.nc'
...
run_name = 'Wuhu_IonTr_run'             #Set to the relevant run name to change input files. Currently working for the runs below:
...
for set_idx in range(len(ds_months_sets)):  #loop through seasons
    ...
    for ids in ds_months_ar:  #loop through months in season
        ...
        if run_name=='Jianfei_run':
            ...
            file1name=f'SpE_Identification_Notebooks/Nc_files/ACP_CESM213_FX2000_f19_f19_mg16_Na_Fe_Mg_iontransport.cam.h2.0001-01-{ds_months[ids]}-00000.nc'
...
#////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            Mptdens_std = np.zeros((Mptdens_sh.shape[0], Mptdens_sh.shape[2], Mptdens_sh.shape[3], Mptdens_sh.shape[4], Mptdens_sh.shape[5]))
            for i in range(Mptdens_sh.shape[0]):
                Mptdens_std[i] = np.std(Mptdens_sh[i], axis=0)
